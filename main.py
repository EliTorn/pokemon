...
def scrape_pokemon_details(pokemon_url):
    ...
    new_pokemon = PokemonData(id=int(id_), name=name, types=types, height=height, weight=weight, abilities=abilities,
                              evolution_url=evolution_url, img=img)
...
for index, pokemon_url in enumerate(pokemon_urls, start=1):
    ...
    pokemon_dict = pokemon_data.__dict__
    pokemon_data_with_key = {"pokemon": pokemon_dict}
    # Save data to a JSON file inside the folder
    file_path = os.path.join(folder_name, f'pokemon_{index}')
    with open(f"{file_path}.json", 'w') as file:
        json.dump(pokemon_data_with_key, file, indent=2)  # Use the data with the added key
    response = requests.get(pokemon_data.img)
